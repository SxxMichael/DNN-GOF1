...
#-------------------------------------------------------------------------------------------------------------------#
def generate_data(nN = 100, n_x_true = 3, n_y_true = 1, low = -1, high = 1):
    X_mat = np.random.uniform(low = low, high = high, size = nN*(n_x_true+1)).reshape((n_x_true+1, nN))
    Y_true = np.absolute(X_mat[1,:]) + 2*np.power(X_mat[2,:],2) + np.cos(2 * pi * X_mat[3,:])
...
#-------------------------------------------------------------------------------------------------------------------#
def main(n_sample, tag = 0, nS = 5, gamma = 0.9, sigma = 0.5, max_hidden_unit = 18,
         deg = 1/3, optimizer = 'sgd', epochs_nn = 200, U = 1,
         epochs_dnn=200, batches = 20,
         early_stopping = False, validation_split = 0.2, 
         patience_nn = 5, patience_dnn = 10, min_delta = 1e-4, plus = False, drop_rate = 0.2,
         verbose = 0, mult = 3/4):
